...
production orders, nor the current status of machines (running, paused).
...
Systemes~\cite{dassault_apriso}, designed for large-scale industrial enterprises.  It covers the full spectrum
...
\begin{itemize}
	...
	\item \textbf{A Node.js middleware layer}: a reverse-proxy Express 5.x server
	      handling SSPI/Windows authentication toward Business Central and exposing
	      a unified REST API to both the Flutter client and the Python AI agent.
	\item \textbf{A Python AI agent (FastAPI)}: provides the conversational
	      intelligence layer, intent classification, multi-provider
	      LLM synthesis, fuzzy machine resolution, and contextual redirect actions.
\end{itemize}
...
\begin{itemize}
	...
	\item Allow operators to start, pause and complete operations from an intuitive
	      reactive interfaces.
	\item Record production declarations (produced quantities, scrap, scan) and synchronise
...
Agile is an iterative and collaborative working method~\cite{agilemanifesto} that takes into account the
...
We adopted the Scrum framework~\cite{scrumguide}, an agile approach based on short sprints and regular

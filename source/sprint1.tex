...
The \uilabel{Paring Page}, shown in Figure~\ref{fig:s1-ui-paring}, is the very
...
The user row in the \uilabel{MES User List Page} includes a
\texttt{PopupMenuButton} action menu accessible via a trailing icon on each row.
The menu exposes the following actions: \uilabel{Change Role / Department},
\uilabel{Generate Password}, \uilabel{View Badge} (active users only),
and \uilabel{Activate} or \uilabel{Deactivate} (label switches dynamically based
on the user's current \texttt{isActive} flag). Selecting \uilabel{Deactivate}
immediately revokes all active sessions for that account; selecting
\uilabel{Activate} re-enables login.

...
work-centre assignment, and activate or deactivate accounts. The sprint also
delivers two-factor authentication via badge QR~code scanning, administrator
badge management (view, print, regenerate), a configurable password expiry
policy, and the first-launch host pairing screen that connects the Flutter
application to the correct \techname{Node.js} middleware instance.

By the end of this sprint, a fully functional \uilabel{login} screen,
\uilabel{badge scan} flow, \uilabel{change password} flow,
\uilabel{admin user management dashboard}, \uilabel{role management dialog},
\uilabel{settings page}, \uilabel{paring page}, and \uilabel{logout}
functionality will all be in place.
...
and stored in the \modulename{MES Auth Token} table. When \domainterm{2FA} is
enabled, the backend returns the \texttt{twoFAEnabled} flag and the
\techname{Flutter} client suspends the session until the badge QR~code is
verified via the \texttt{VerifyBadge} endpoint.
...
a PDF (with landscape/portrait toggle), and regenerate the underlying secret

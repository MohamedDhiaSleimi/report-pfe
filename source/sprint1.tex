...
%     participant BC
%
%     Flutter->>AL: any protected request (token, payload)
%     AL->>BC: Get MES Auth Token
%     BC-->>AL: token record
%     AL->>AL: Check Validity
%     AL->>BC: Get MES User (token.UserId)
%     BC-->>AL: MES User record
%     AL->>AL: Check IsActive = true
...
%     Flutter->>AL: Logout(token)
%     AL->>BC: Get MES Auth Token where Token = GUID
%     BC-->>AL: token record
%     AL->>BC: Set Revoked = true, Modify
%     BC-->>AL: ok
%     AL-->>Flutter: {success: true}
...
%     User->>Flutter: Fill AddUserDialog and submit

...
\begin{longtable}{@{} P{0.09\linewidth} P{0.34\linewidth} P{0.53\linewidth} @{}}
      ...
      \begin{itemize}[nosep, leftmargin=*]
            ...
            \item Create \techname{LogService} to consume the \texttt{fetchMachineDashboard} endpoint and \techname{LogProvider} to manage its state.
      ...
      \begin{itemize}[nosep, leftmargin=*]
            ...
            \item Create \techname{LogService} to consume the \texttt{fetchActivityLog()} endpoint and \techname{LogProvider} to manage its state.
	...
	so that I have a source of the mes barcodes labels.
      & \noindent\textit{Business Central (AL):}
      \begin{itemize}[nosep, leftmargin=*]
            \item Create function \texttt{fetchAllItemBarcodes()} in codeunit \techname{MES Machine Fetch}.
            \item Add the wrapper of the function to \techname{MES Web Service}.
      \end{itemize}
	\\ & & \noindent\textit{Flutter:}
      \begin{itemize}[nosep, leftmargin=*]
...
operator, machine, time, order, operation number and action ; AppBar dropdowns filter by type and 
...
administration layer and cross-cutting quality-of-life features. \userrole{Supervisor}
can now monitor the production health of the entire facility through a
configurable machine performance dashboard, and the \userrole{Administrator} can audit all operator and machine
actions through a paginated activity log, view the barcodes of all the items, and manage users. All users benefit from

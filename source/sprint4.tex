...
Administrators receives access to an activity log, barcode list, and a user role management
...
\begin{longtable}{@{} P{0.09\linewidth} P{0.34\linewidth} P{0.53\linewidth} @{}}
  ...
  \begin{itemize}[nosep, leftmargin=*]
    \item Create function \techname{fetchMachineDashboard()} in codeunit \techname{MES Machine Fetch}.
  ...
  \begin{itemize}[nosep, leftmargin=*]
    ...
    \item Create \techname{LogService} to consume the \techname{fetchMachineDashboard} endpoint and \techname{LogProvider} to manage its state.
  ...
  & \noindent\textit{Business Central (AL):}
  \begin{itemize}[nosep, leftmargin=*]
    \item Create function \techname{fetchActivityLog()} in codeunit \techname{MES Machine Fetch}.
  ...
  \begin{itemize}[nosep, leftmargin=*]
    ...
    \item Create \techname{LogService} to consume the \techname{fetchActivityLog()} endpoint 
  \end{itemize} \\ & &
  \begin{itemize}[nosep, leftmargin=*]
    \item Create \techname{LogProvider} to manage its state.
  ...
  & \noindent\textit{Flutter:}
  \begin{itemize}[nosep, leftmargin=*]
    \item Integrate the \techname{easy\_localization} package.
    \item Create translation JSON files for \techname{en}, \techname{fr}, and \techname{ar}.
    \item Create widget \techname{LanguageSelector}.
    \item Implement language selection in \techname{ProfilePage}.
    \item Add the \techname{EasyLocalization} wrapper in \techname{main()}.
    \item Implement right-to-left layout support through \techname{MaterialApp}.
  \end{itemize} \\
  \midrule
  \textbf{US-4.4}
  ...
  & \noindent\textit{Flutter:}
  \begin{itemize}[nosep, leftmargin=*]
    \item Integrate the \techname{tutorial\_coach\_mark} package.
  ...
  & \noindent\textit{Business Central (AL):}
  \begin{itemize}[nosep, leftmargin=*]
    \item Create function \techname{fetchAllItemBarcodes()} in codeunit \techname{MES Machine Fetch}.
    \item Add the wrapper of the function to \techname{MES Web Service}.
  \end{itemize}
  \\ & & \noindent\textit{Flutter:}
  \begin{itemize}[nosep, leftmargin=*]
    \item created the page \techname{BarcodeListPage.dart}.
    \item updated the povider mes\_barcode\_provider.dart and mes\_barcode\_service.dart to consume the new endpoints.
    \item created the widget dataMatrix\_card.dart.
  \end{itemize} \\
\end{longtable}
% ---------------------------------------------------------------------------
% 2. BUSINESS RULES
...
interaction \uilabel{View Activity Log} and \uilabel{view item barcodes}.The \userrole{Supervisor} gains the interaction
...
\begin{longtable}{@{} P{0.21\linewidth} P{0.75\linewidth} @{}}
      ...
      totals, finished operation and concelled/interrupted operations and scrap for the selected time window.
      ...
      with the default \techname{hoursBack = 24}.
...
records and, for each machine, computes the total finished operation count, cancelled/Interrupted operations,
...
\begin{figure}[H]
      ...
      \caption{SD-19: Machine Dashboard.}
...
\domainterm{MES Operation Scrap}, and \domainterm{MES Operation Scan}
...
\techname{fetchMachineDashboard()} accepts a \techname{hoursBack} window and an
optional \techname{workCenterNoJson} filter. For each Machine Center, it aggregates 
execution data, counts finished and cancelled operations, calculates produced and 
scrap quantities within the selected time window, and computes uptime from machine 
status records as the percentage of time spent in the Working state.

\subsubsection*{Activity Log Aggregation}


\techname{fetchActivityLog()} queries four tables within the \techname{hoursBack}
...
the inline \techname{http.post} calls from Sprint~1.
...
\techname{parseList()} and \techname{parseObject()} decode OData list and
single-object responses respectively; \techname{parseWriteResult()} decodes
write responses and surfaces backend errors; \techname{\_assertSuccess()}
...
\begin{longtable}{@{} P{0.30\linewidth} P{0.65\linewidth} @{}}
  ...
  \techname{LogProvider}        &
   Owns dashboard and activity log fetch calls; exposes \techname{selectedHours},
   \techname{hourOptions}, and \techname{setHours()} shared between the two admin pages. \\
  \addlinespace
  \techname{MesBarcodeProvider} & 
  Exposes \techname{fetchAllBarcodes()}; resolves the session token automatically from
...
Figures~\ref{fig:s4-ui-machine-dashboard}, ~\ref{fig:s4-ui-barcode}, and~\ref{fig:s4-ui-activity-log} illustrate
the three pages. The \uilabel{Machine Dashboard Page} renders one card per machine
...
operator, declared by, machine, time, order, operation number and action ; AppBar dropdowns filter by type and 
time range. The \uilabel{Barcode Page} presents a searchable list of all items with their associated
barcodes rendered as DataMatrix codes.
...
\begin{figure}[H]
  ...
  \caption{\uilabel{Activity Log Page}.}
  \label{fig:s4-ui-activity-log}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[height=0.3\textheight,keepaspectratio]{./media/MES_Sprint-4-Report/UI/barcode/wide.png}
  \caption{\uilabel{Barcode Page} --- desktop and mobile views.}
  \label{fig:s4-ui-barcode}
\end{figure}
...
\begin{figure}[H]
  ...
  \colorbox{black}{%
    \begin{tabular}{@{}c@{}}
      \includegraphics[height=0.3\textheight,keepaspectratio]{./media/MES_Sprint-4-Report/UI/translation/french.png}\\[2pt]
      \includegraphics[height=0.3\textheight,keepaspectratio]{./media/MES_Sprint-4-Report/UI/translation/arabic.png}
    \end{tabular}%
  }
  \caption{\uilabel{Machine Dashboard Page} in both French and Arabic.}
  \label{fig:s4-ui-translation-dashboard}
\end{figure}
...
This sprint completed the \domainterm{MES} solution by delivering the administration
layer and cross-cutting quality-of-life features. \userrole{Supervisor} can now monitor
the production health through a configurable machine performance dashboard, and the 
\userrole{Administrator} can audit all operator and machine actions through a paginated
activity log, view the barcodes of all the items, and manage users. All users benefit 
from full internationalisation covering English, French, and Arabic with automatic 
right-to-left layout, and from guided coach-mark tutorials that ease onboarding for 
first-time users.

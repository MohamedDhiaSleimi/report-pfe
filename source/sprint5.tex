...
\begin{longtable}{@{} P{0.28\linewidth} P{0.67\linewidth} @{}}
	...
	\texttt{operation\_bom} & \textit{bom}, \textit{component}, \textit{material}, \textit{consumption}, \textit{scan} \\
	\midrule
	...
	\midrule
	\texttt{machine\_history} & \textit{history}, \textit{past operation}, \textit{finished}, \textit{cancelled} \\
	\midrule
	\texttt{machine\_orders} & \textit{order}, \textit{production order}, \textit{PO-}, \textit{scheduled} \\
	\midrule
	\texttt{machine\_live} & \textit{machine status}, \textit{what is MC-}, \textit{ongoing} \\
	\midrule
	\texttt{machine\_dashboard} & \textit{dashboard}, \textit{summary}, \textit{overview}, \textit{KPI} \\
	\midrule
	...
	\midrule
	\texttt{activity\_log} & \textit{activity}, \textit{event log}, \textit{what happened}, \textit{recent actions} \\
	\midrule
	\texttt{department\_overview} & \textit{work center}, \textit{department}, \textit{sector}, \textit{operator summary} \\
	\midrule
...
We configured the middleware to map all calls coming from the frontend using
the structure \techname{/api/ai} to the AI proxy handler, which registers two
routes: \texttt{POST /api/ai/chat} forwards requests to the \texttt{/chat}
endpoint of the Python agent, and \texttt{GET /api/ai/health} proxies health
checks to the agent's health endpoint. The agent's tool calls are treated
through the same path as the normal Business Central function calls from the
Flutter frontend.

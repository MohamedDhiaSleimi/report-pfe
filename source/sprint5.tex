...
\begin{longtable}{@{} P{0.30\linewidth} P{0.65\linewidth} @{}}
	...
	When a user refers to a machine by a colloquial name or partial ID, \modulename{resolver.py} applies a five-tier match cascade. A confidence score below \texttt{MIN\_CONFIDENCE = 0.45} causes the assistant to ask the user to clarify. A score between 0.45 and \texttt{HIGH\_CONFIDENCE = 0.85} causes the assistant to proceed while noting the assumption. A score at or above 0.85 causes silent resolution. The five tiers are: (1) exact canonical ID, (2) normalised exact, (3) digit suffix match, (4) ID prefix/suffix overlap, and (5) token overlap and edit-distance scoring on the machine name. \\
	\midrule
...
\begin{longtable}{@{} P{0.25\linewidth} P{0.20\linewidth} P{0.50\linewidth} @{}}
...
\begin{longtable}{@{} P{0.32\linewidth} P{0.30\linewidth} P{0.33\linewidth} @{}}
	...
	\midrule
	\techname{GetOngoingOperationsTool} & \texttt{fetchOngoingOperationsState} & \texttt{machine\_no} \\
	\midrule
...
The middleware maps incoming \texttt{/api/<endpoint>} requests to BC URLs using two lookup structures. The \texttt{webServiceEndpoints} \texttt{Set} contains all 38 MES web service names; matching paths are forwarded to \texttt{<webServiceBase><endpoint>?company=<BC\_COMPANY>}. The \texttt{apiBaseEndpoints} object maps three API collection names (\texttt{scrapCodes}, \texttt{employees}, \texttt{workCenters}) to their corresponding OData API paths. Unrecognised paths fall back to \texttt{BC\_HOST + originalUrl} with the \texttt{/api} prefix stripped.

\subsubsection{PowerShell SSPI Bridge}

Each request to BC is handled by \texttt{runPowerShell()}, which spawns a \texttt{powershell.exe} child process and feeds it a script produced by \texttt{buildPsScript()}. The script creates an \texttt{HttpClientHandler} with \texttt{UseDefaultCredentials = \$true}, sends the HTTP request, reads the response bytes, and emits a JSON object containing \texttt{status}, \texttt{headers}, and \texttt{bodyBase64} to standard output. Node reads the last line beginning with \texttt{\{} from stdout (tolerating any PowerShell informational messages before it), parses the JSON, filters the response headers through the \texttt{SKIP\_RESPONSE} set, and forwards the decoded body to Flutter. A kill timer expires \texttt{BC\_TIMEOUT\_MS + 2000} ms after the process is spawned.

\subsubsection{Provider Architecture}

\begin{longtable}{@{} P{0.28\linewidth} P{0.67\linewidth} @{}}
	\caption{Node.js Middleware Module Responsibilities}
	\label{tab:s5-middleware-modules} \\
	\toprule
	...
	\modulename{index.js} & Bootstraps Express, composes the middleware pipeline (CORS → body parser → health → OPTIONS preflight → AI router → BC catch-all → error handler), and starts the HTTP server. \\
	\midrule
	\modulename{src/config.js} & Loads and validates all environment variables at startup. Computes derived URL fragments (\texttt{odataBase}, \texttt{webServiceBase}, \texttt{apiBase}, \texttt{companyParam}). Fails fast on missing required variables. \\
	\midrule
	\modulename{src/cors.js} & Exports CORS options: whitelist-based origin validation, credentials enabled, \texttt{maxAge} 3600 s. Requests with no \texttt{Origin} header (server-to-server) are unconditionally allowed. \\
	\midrule
	\modulename{src/proxy.js} & Implements \texttt{buildTargetUrl()}, \texttt{sanitizeRequestHeaders()}, \texttt{buildPsScript()}, \texttt{runPowerShell()}, and the main \texttt{forwardRequest()} Express handler. Emits structured request/response log lines to stdout. \\
	\midrule
	\modulename{src/aiProxy.js} & Express Router mounted at \texttt{/api/ai}. Implements \texttt{POST /chat} (validates input, forwards to Python agent with \texttt{AbortController} timeout, handles 400/502/504 errors) and \texttt{GET /health} (probes agent with 3 s timeout). \\
	\midrule
	\modulename{src/logger.js} & Reserved stub. All current logging is performed via inline \texttt{console.log} calls in \modulename{proxy.js} and \modulename{aiProxy.js}. \\
\end{longtable}

...
\begin{longtable}{ p{1.5cm} p{5cm} p{6.5cm} p{2cm} }
	...
	the same order is not either finished, interupted after the its start, or running but produced enough for the send ahead quantity.
	\domainterm{Running}.
	\end{itemize} & High \\ \midrule
	...
	\item Each card shows order number, operation number, current status, operation description, and
	...
	\item The page provides at least two tabs: \domainterm{Orders},
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	\item Cancelling either an incomplete or unstarted order shows a red warning dialog.
	...
	&
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, leftmargin=*]
	\item Components are colour-coded by scanned amount (the needed amount (green) / less then necessary to make a single item (red) otherwise (yellow)).
	...
	&
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, leftmargin=*]
	\item any scanner connected to the device reads  barcodes.
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	Activity Logs, system barcodes and Settings.

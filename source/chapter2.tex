...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	\item Camera opens and reads a QR badge code automatically.
	...
	\item Print button generates a PDF with landscape/portrait toggle.
	\item Regenerate action shows a confirmation dialog before replacing the secret.
	\item New QR is displayed immediately after regeneration without a page reload.
	\item Actions are available only for active users and to the \userrole{Admin} role.
	\end{itemize}
	&
	High \\ \midrule
	...
	&
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, leftmargin=*]
	\item \domainterm{2FA} toggle and password period field available on the \uilabel{Settings} page.
	\item Changes take effect on the next login attempt.
	\item Settings are persisted in the \techname{MES Settings} table in \techname{Business Central}.
...
\begin{longtable}{ p{1.5cm} p{5cm} p{6.5cm} p{2cm} }
	...
	\domainterm{Working}), and active production
	order number.
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	\item Fan-out is capped at 12 machines to control response time.
	...
	\item Supported targets: machine detail, operation detail, machine list,
	dashboard, and operation history.
...
\begin{longtable}{@{} P{0.10\linewidth} P{0.30\linewidth} P{0.55\linewidth} @{}}
	...
	role and work-centre change, account activation/deactivation,
	two-factor authentication with badge QR~code, badge management
	(view, print, regenerate), password expiry policy configuration,
	and first-launch host pairing. \\ \midrule
	...
	in-app onboarding tutorials. \\ \midrule
...
\begin{itemize}
	...
	\item \textbf{Enums (2-Enum)}: define the enumerated types --- MES User Role
	      (Operator, Supervisor, Admin), MES Machine Status (Idle, Working),
	      MES Operation Status (Running, Paused, Finished, Cancelled, Interrupted).
	\item \textbf{Codeunits (3-CodeUnit)}: contain the business logic --- MES Auth Mgt
	      (authentication, tokens), MES Password Mgt (SHA-256 hashing), MES Machine Fetch
	      and MES Machine Write (machine and operation management), MES Unbound Actions (OData V4 facade).

...
\begin{longtable}{@{}P{0.21\linewidth}P{0.75\linewidth}@{}}
	...
	orders. Mark operation as finished or cancel/interupt uneeded operations, And they also have access to all of the operator's permissions. \\[6pt]
	...
	start, pause or resume an operation, declare produced quantities, defective materials, and scan material barcodes to declare their use. \\
...
\begin{itemize}
	...
	\item \textbf{Production Order Management}: This feature allows the operator and the supervisor to:
	      \begin{itemize}
	...
	\item \textbf{Production Operation Management}: This feature is the core of the
	      ...
	      \begin{itemize}
		 	...
		 	\item Declare produced quantities (validation: quantity $> 0$, no excess of the ordered quantity).
	      	\item View progress in real time (produced quantity, completion percentage, operation status, scrap).
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	\item Clicking on the action button shows a dropdown menu with available actions.
...
\begin{longtable}{ p{1.5cm} p{5cm} p{6.5cm} p{2cm} }
	...
	\domainterm{Working}), active production order number, operation No, and
	the item name.
	...
	\item A new MES Operation State record is created with status.
	\item An operation cannot be started if the previous operation in 
	the same order is not started.
	\domainterm{Running}.
	\end{itemize} & High \\ \midrule
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	\item If the operation is started then it is marked as cancelled otherwise as cancelled
	...
	\item shows only the components linked to the operation with the routing link code
	\end{itemize}
	& High \\ \midrule
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	&
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, leftmargin=*]
	\item Sidebar has sections for Users \& Roles,
...
\begin{longtable}{@{} P{0.10\linewidth} P{0.30\linewidth} P{0.55\linewidth} @{}}
	...
	and session token management, role-based access control,
	...
	(view, print, regenerate), password expiry policy configuration, export users
	...
	Monitoring dashboard and Administrator activity log, multi-language support
	(EN/FR/AR) and RTL layout, in-app onboarding tutorials. \\ \midrule
...
\begin{itemize}
	\item \textbf{Tables (1-Tables)}: define the persistent data model ---
		  ...
		  MES Operation Scrap, MES Operation Execution, MES Operation
	      Progression, MES Settings, MES Component Consumption, MES User Execution Interaction, .
	...
	\item \textbf{Codeunits (3-CodeUnit)}: contain the business logic --- MES Auth Mgt
	      ...
	      (read operations), MES Machine Write (state orchestration), MES Machine Insert
		  (table writes), MES Machine Validation (business rule guards), MES Unbound Actions.
	\item \textbf{API Pages (4-API)}: expose data for reading/writing via the BC REST
	      API --- employees, work centres.
	\item \textbf{Pages (5-Pages)}: administration and debugging interfaces in the BC
	      client (lists, cards, API debug page, etc...).
\end{itemize}

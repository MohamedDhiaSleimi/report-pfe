...
\begin{longtable}{@{}P{0.21\linewidth}P{0.75\linewidth}@{}}
	...
	passwords and account statuses, etc. They have access to all
	administration application dashboards. \\[6pt]
	\midrule
	Supervisor     &
	An intermediate user who is a superset of the Operator role. In addition to their 
	own access, they inherit all Operator permissions. They can monitor the progress 
	of production operations on their work centre, view machine status, and track 
	the progress of production orders. They can mark operations as finished, cancel, 
	or interrupt unneeded operations, view machine dashboards, and declare on behalf 
	of an Operator. \\[6pt]
	\midrule
	Operator       &
...
\begin{itemize}
	...
	\item \textbf{User Management}: This feature allows the administrator to:
	      \begin{itemize}
	      	\item View the list of MES users.
	      	\item Export the list of users.
	      	\item Create a new user account by linking it to a Business
	      	      Central employee and assigning them a role and work centre(s).
	...
	\item \textbf{Production Operation Management}: This feature is the core of the
	      ...
	      \begin{itemize}
		 	...
		 	\item Declare produced quantities (validation: quantity $> 0$, enough scanned component, no excess of the ordered quantity, etc).
	...
	\item \textbf{Progress Dashboard}: Allows the supervisor to view an overview of all machines information 
	 (uptime, operation completed, produced units, etc)
...
\begin{itemize}
	...
	\item \textbf{Portability}: The Flutter application is compiled for Android,
...
the following Figure~\ref{fig:mescha-global-uc} dipects the global use case diagram of the MES system, illustrating the interactions between the various user roles and the system's functionalities.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth,keepaspectratio]{./media/MES_chapter-2-repport/global-useCase.png}
	\caption{MES System Global Use Case Diagram}
	\label{fig:mescha-global-uc}
\end{figure}


\section{Backlog Features}

% ===================== DOMAIN 1: AUTHENTICATION & USER MANAGEMENT =====================
\subsubsection*{Feature 1: Authentication \& User Management}
\label{domain1}
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	&
	\begin{itemize}[nosep, leftmargin=*]
	\item User can enter Auth-ID and password.
	...
	& \begin{itemize}[nosep, leftmargin=*]
	\item The user list displays each account's information.
	...
	\item The generated password is sent to the \techname{MES} via \techname{AdminSetPassword}.
	...
	that my identity is verified.
	&
	\begin{itemize}[nosep, leftmargin=*]
	\item \domainterm{2FA} prompt appears only when \domainterm{TwoFA Enabled}.
	...
	\textbf{US-1.11} &
	As a user, I need to configure a host URL
	so that the application connects to my company's server.
	&
	\begin{itemize}[nosep, leftmargin=*]
	...
	& \begin{itemize}[nosep, leftmargin=*]
	\item The ability to export users.
...
\subsubsection*{Feature 2: Machine \& Production Management}
\label{domain2}
...
\begin{longtable}{ p{1.5cm} p{5cm} p{6.5cm} p{2cm} }
	...
	\item Each card displays order number, product description, planned quantity,
	planned start and end dates, and operation description.
	\end{itemize} &
	High \\
	\midrule
	...
	production operation so that the system reflects the change.
	& \begin{itemize}[nosep, leftmargin=*]
	...
	\domainterm{Finished}, \domainterm{Cancelled}, \domainterm{Interrupted}, or currently \domainterm{Running}
	...
	for a machine so that I can navigate between pending orders, active
	operation progress and the operation history of the machine without leaving the machine context.
	& \begin{itemize}[nosep, leftmargin=*]
	...
	sort production orders so that I can quickly find the relevant work.
	& \begin{itemize}[nosep, leftmargin=*]
	...
	\bottomrule
\end{longtable}
\subsubsection*{Feature 3: Production Execution \& Traceability}
\label{domain3}
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	produced quantity in a cycle so that the system records my progress.
	&
	\begin{itemize}[nosep, leftmargin=*]
	...
	&
	\begin{itemize}[nosep, leftmargin=*]
	\item Finishing a complete order (100\%) shows a confirmation dialog.
	\item Interrupting or Cancelling an operation shows a warning dialog.
	...
	production cycles declared for an operation so that I can review each declaration's information.
	&
	\begin{itemize}[nosep, leftmargin=*]
	...
	\item BOM data updates after every consumption event.
...
\subsubsection*{Feature 4: Administration \& Monitoring}
\label{domain4}
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	\midrule
	\textbf{US-4.5} &
	As an \userrole{Administrator}, I need a searchable barcode page
	so that I have a source of the mes barcodes labels.
	&
	\begin{itemize}[nosep, leftmargin=*]
	\item the barcode page is searchable.
	\item Ability to print out the barcode labels.
	\item the ability to reload the barcode list.
	\end{itemize}
	& Medium \\
	\bottomrule
\end{longtable}
...
\subsubsection*{Feature 5: AI Chat Assistant \& Analytical Intelligence}
\label{domain5}
...
\begin{longtable}{p{1.5cm} p{5cm} p{6.5cm} p{2cm}}
	...
	chat assistant so that I can query shop floor status without navigating the full UI.
	&
	\begin{itemize}[nosep, leftmargin=*]
	...
	answer fleet-wide questions so that I
...
\begin{longtable}{@{} P{0.10\linewidth} P{0.25\linewidth} P{0.45\linewidth} P{0.15\linewidth} @{}}
	\caption{MES project sprint plan}
	\label{tab:mescha-sprint_plan} \\
	\toprule
	\textbf{Sprint} & \textbf{Domain} & \textbf{Scope} & \textbf{period} \\
	\midrule
	\endfirsthead
	...
	supervisor proxy declaration, and label printing. 
	& 5 weeks \\ \midrule

	Sprint~4        & Administration and System Monitoring \& Configuration              &
	Monitoring dashboard and Administrator activity log, barcode view and printing, 
	multi-language support (EN/FR/AR) and RTL layout, in-app onboarding tutorials. 
	& 2 weeks \\ \midrule
...
The physical architecture of the MES system rests on three main nodes:
	...
	\begin{itemize}
	    ...
	    follow the schema
	    \techname{api.businesscentral.dynamics.com/v2.0/<tenantId>/<environment>/ODataV4/}.

	    \item \textbf{Node.js Middleware}~\cite{nodejs,express}: a reverse-proxy layer running between the Flutter
	    ...
	    \item \textbf{Flutter Client (mobile / web / desktop)}~\cite{flutter}: application compiled for
	    Android, Web or desktop, communicating with the Node.js middleware via HTTP
	    requests. The middleware proxies these to the BC OData V4 endpoints.
		
		\item \textbf{A Python AI agent (Server)}~\cite{fastapi,python}: a module that acts as an interface between
...
\begin{itemize}
	...
	MES Settings, MES Operation Scan, MES User Work center, and MES User Execution Interaction.
...
\subsubsection*{Node.js Middleware}
	...
	AI chat requests (\techname{POST /api/ai/chat}) to the Python AI agent.
...
\begin{itemize}
	\item \textbf{Data}: contains data models (models) and HTTP services (services) that
	      communicate with BC endpoints via the \techname{http} library.
	...
	\item \textbf{Presentation}: contains the pages and widgets of the user interface,
	      organised by functional domain (auth, admin, machine, etc) with dedicated responsive
	      layouts.
\end{itemize}
...
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.09\linewidth}>{\centering\arraybackslash}m{0.38\linewidth}>{\centering\arraybackslash}m{0.49\linewidth}@{}}
	...
	Microsoft Dynamics 365 Business Central~\cite{businesscentral} &
	...
	Flutter (Dart)~\cite{flutter,dart_lang}&
	...
	Visual Studio Code&
	Primary development environment (AL) \\[6pt]
	\midrule
	...
	\midrule
	\includegraphics[width=\linewidth]{./media/MES_chapter-2-repport/android-studio-icon.png}&
	Android Studio&
	Primary development environment (Flutter)  \\[6pt]
	\midrule
\end{longtable}

\subsection{Development Environment}

The project was developed on two workstations: the first machine equipped with an
11\textsuperscript{th} Gen Intel
i5 processor, and the second machine with a
13\textsuperscript{th} Gen Intel
i7 processor. Both had  16\,GB RAM and  ran a 64-bit
\techname{Windows} operating system.

The software toolchain consisted of \techname{Flutter 3.41.0} and \techname{Dart 3.11.0}
for the mobile frontend, \techname{Node.js v24} for the middleware layer,
\techname{Python 3.14} with \techname{FastAPI} for the AI agent, and
\techname{Microsoft Dynamics 365 Business Central NAV 22} as the ERP backend.
The extensions \techname{AL Language}, \techname{Flutter}, and \techname{Dart} 
were installed in \techname{Visual Studio Code}.

The on-premises \techname{Business Central} instance was configured to expose both
the \domainterm{OData V4} and \domainterm{API Services} endpoints, which are consumed
by the middleware and the mobile frontend.

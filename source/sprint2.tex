...
\modulename{MES Machine Status} (Enum 50101) defines two values:
\domainterm{Idle} and \domainterm{Working}.
\modulename{MES Operation Status} (Enum 50102) defines five values:
...
Machine and operation business logic is encapsulated across two codeunits:
\modulename{MES Machine Fetch} (50131) for read operations and
\modulename{MES Machine Write} (50132) for write operations. Their public
procedures are called directly by the OData web service layer.


\textbf{FetchMachines.}
Accepts a \domainterm{JSON array of work centre numbers} (\texttt{workCenterNoJson})
and returns a \domainterm{JSON} array
...
lines that already have an existing \modulename{MES Operation Execution} record are
...
\modulename{MES Operation State} record with status \domainterm{Running} is
...
(Machine No, Prod Order No, Operation No, Declared At) and iterates the
...
established in Sprint~1.The procedures are exposed through the existing \modulename{MESWebService}
(codeunit 50126) registered in \modulename{WebServices.xml}. After deployment,
...
\begin{minipage}[t]{0.38\textwidth}
  \vspace{0pt}
  \centering
  \includegraphics[width=\linewidth]{./media/MES_Sprint-2-Repport/placeholder-image.png}
  \captionof{figure}{\techname{Flutter} machines domain --- layered architecture.}
  \label{fig:s2-flutter-architecture-s2}
\end{minipage}


20-second \texttt{Future.delayed} delay, emitting an updated list on every cycle and
